...
public class WoodChisel extends ToolItem {

    public static final Identifier PACKET_ID = new Identifier(BuilderHelper.MOD_ID, "wood_chisel_packet");
    private long lastBreakTick = 0;

    public WoodChisel(Settings settings) {
    ...
    public void init() {
        ServerSidePacketRegistry.INSTANCE.register(PACKET_ID, (packetContext, attachedData) -> {
            ...
            packetContext.getTaskQueue().execute(() -> {
                ...
                if (world.canSetBlock(pos) && stack.getItem() == BACCompat.getInstance().WOOD_CHISEL_ITEM && player.getBlockPos().getSquaredDistance(pos.getX(), pos.getY(), pos.getZ(), true) < 81) {
                    for (int i = 0; i < 8; i++) {
                        for (int j = 0; j < 8; j++) {
                            for (int k = 0; k < 8; k++) {
    ...
    public void initClient() {
        AttackBlockCallback.EVENT.register((player, world, hand, pos, direction) -> {
            ...
            if (i instanceof WoodChisel) {
                return ((WoodChisel)i).interactBreakBlockClient(player, world, pos);
            }
        ...
        RedBoxCallback.EVENT.register((redBoxDrawer, matrixStack, vertexConsumer, worldoffsetx, worldoffsety, worldoffsetz) -> {
            ...
            if (client.player.getMainHandStack().getItem() == BACCompat.getInstance().WOOD_CHISEL_ITEM) {
                HitResult hit = client.crosshairTarget;
                if (hit.getType() == HitResult.Type.BLOCK) {
                    Direction direction = ((BlockHitResult)hit).getSide();
                    BlockPos pos = ((BlockHitResult)hit).getBlockPos();
                    int x = (int) Math.floor(Math.floor(((hit.getPos().getX() - pos.getX()) * 16) + (direction.getOffsetX() * -0.5d)) / 8) * 8;
                    int y = (int) Math.floor(Math.floor(((hit.getPos().getY() - pos.getY()) * 16) + (direction.getOffsetY() * -0.5d)) / 8) * 8;
                    int z = (int) Math.floor(Math.floor(((hit.getPos().getZ() - pos.getZ()) * 16) + (direction.getOffsetZ() * -0.5d)) / 8) * 8;
                    for (int i = 0; i < 8; i++) {
                        for (int j = 0; j < 8; j++) {
                            for (int k = 0; k < 8; k++) {
