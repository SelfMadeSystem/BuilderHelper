package uwu.smsgamer.builderhelper.screens;
...
import net.minecraft.client.search.*;
...
public class BlockScreen extends AbstractInventoryScreen<BlockScreen.CreativeScreenHandler> {
    ...
    private  Function<ItemStack, Boolean> clickFunction;

    public BlockScreen(PlayerEntity player, Function<ItemStack, Boolean> clickFunction) {
    ...
    protected void onMouseClick(@Nullable Slot slot, int invSlot, int clickData, SlotActionType actionType) {
        ...
        if (slot == null && selectedTab != ItemGroup.INVENTORY.getIndex() && actionType != SlotActionType.QUICK_CRAFT) {
            ...
            if (slot != this.deleteItemSlot) {
                if (clickFunction.apply(slot.getStack())) {
                    this.client.openScreen(null);
                }
                // this.client.player.sendMessage(new LiteralText("Hi. " + slot.getStack().getItem().getName().getString()), false);
            }
            ...
            /*if (slot == this.deleteItemSlot && bl) {
                ...
                if (selectedTab == ItemGroup.INVENTORY.getIndex()) {
                    if (slot == this.deleteItemSlot) {
                        ...
                        this.client.interactionManager.clickCreativeStack(itemStack3, ((CreativeSlot) slot).slot.id);
                        ...
                        this.client.player.playerScreenHandler.onSlotClick(slot == null ? invSlot : ((CreativeSlot) slot).slot.id, clickData, actionType, this.client.player);
    ...
    private void searchForTags(String string) {
        ...
        tagGroup.getTagIds().stream().filter(predicate2).forEach((identifier) -> {
            Tag var10000 = this.searchResultTags.put(identifier, tagGroup.getTag(identifier));
        });
    }
    ...
    private void setSelectedTab(ItemGroup group) {
        ...
        if (group == ItemGroup.INVENTORY) {
            ...
            for (l = 0; l < screenHandler.slots.size(); ++l) {
                ...
                Slot slot = new CreativeSlot(screenHandler.slots.get(l), l, t, aa);
